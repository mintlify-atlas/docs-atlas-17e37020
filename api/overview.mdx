...
Hagaki Renderer exposes a small HTTP API for rendering card game assets to PNG. All three endpoints follow the same pattern: encode a JSON payload as a base64 URL-safe string, then pass it as a path segment.
...
Each `{hash}` path segment is a base64 URL-safe, no-padding encoding of a JSON payload.

1. Serialise your request struct to JSON.
2. UTF-8 encode the JSON string to bytes.
3. Encode the bytes with base64 standard alphabet, URL-safe variant, **no padding** (`=` stripped).
