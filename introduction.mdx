...
Hagaki exposes three HTTP endpoints. Each accepts a **base64url-encoded JSON hash** that fully describes the render payload and returns an `image/png` response.
...
<Steps>
  ...
  <Step title="Encode the payload as a hash">
    Serialize the JSON to a string and encode it as base64url (no padding). This hash becomes the final segment of the request URL and fully identifies the render.
  </Step>
